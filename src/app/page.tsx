      ...
      <TableBody>
        {matches.map(({date, goals1, goals2, team1, team2}) => (
          <TableRow key={date}>
            ...
            <TableCell className={cn({"font-bold text-green-500": goals1 > goals2})}>{goals1}</TableCell>
            <TableCell className={cn("text-right", {"font-bold text-green-500": goals2 > goals1})}>{goals2}</TableCell>
        </TableRow>
        ))}
      </TableBody>
    </Table>
