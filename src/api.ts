...
const api = {
    match: {
        list: async (): Promise<Match[]> => {
            return fetch("https://docs.google.com/spreadsheets/d/e/2PACX-1vQQwPMJMlOvVhxKB1wrRBvWMGysobxPjFAVKOnRhu3s0J6qCMqYikeECaed_wwjk38xJ0yLu9MOAbZG/pub?output=tsv",
            {next: {tags: ["matches"]}})
            .then((res) => res.text())
            .then(text => {
                return text.split('\n').slice(1).map(row => {
